...
### 事件驱动架构
- **异步事件处理**：基于 Kafka 的事件驱动架构，处理用户生命周期事件
- **事件类型**：用户注册、登录、密码变更、状态变更、删除和更新事件
- **可靠投递**：幂等生产者，支持重试机制和消息确认
- **可扩展处理**：消费者组支持负载均衡和水平扩展
- **可观测性**：Kafka 操作的完整日志、指标和健康检查
- **优雅降级**：事件发布失败不影响主要业务流程
...
应用程序实现了基于 Apache Kafka 的健壮事件驱动架构，用于异步处理用户生命周期事件。

#### 支持的事件类型
- **用户注册**：`user.registered` - 新用户注册时触发
- **用户登录**：`user.logged_in` - 用户成功登录时触发
- **密码变更**：`user.password_changed` - 用户更改密码时触发
- **状态变更**：`user.status_changed` - 用户状态被修改时触发
- **用户删除**：`user.deleted` - 用户账户被删除时触发
- **用户更新**：`user.updated` - 用户资料更新时触发
...
- **可扩展处理**：消费者组支持水平扩展
- **优雅降级**：事件发布失败不影响主要业务流程
- **完整日志**：结构化日志，支持请求 ID 追踪
- **健康监控**：Kafka 连接和消费者组健康检查
...
- **集成指南**：`docs/kafka-integration.md` - 详细的设置和使用指南
- **实现总结**：`KAFKA_IMPLEMENTATION_SUMMARY.md` - 高级架构概述
- **测试脚本**：`scripts/test-kafka.sh` - 自动化 Kafka 功能测试
...
# Kafka 测试
make test-kafka             # 运行 Kafka 集成测试
./scripts/test-kafka.sh     # 手动 Kafka 测试脚本
